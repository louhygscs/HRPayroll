...
var App = function() {
	...
	var handleResponsive = function() {
		...
		$('.navbar li.nav-toggle').click(function() {
			$(this).hide();
			$('.openSidbarIcon .toggle-sidebar').show();
			// $('.openSidbarIcon .toggle-sidebar').css('visibility','visible');
		});
		/**
		...
		/* icon togglign */
		$('.openSidbarIcon a').css('display','none');
		$('.sidebarBrand .toggle-sidebar').click(function (){
			$('#brandContent').css('padding-left','0');
			$('.openSidbarIcon a').css('display','block');
			
		});
		
		$('#brandContent .toggle-sidebar').click(function (){
			$('#brandContent').css('padding-left','225px');
			...
			if ($('#sidebar').hasClass('sidebar-fixed') || $(window).width() <= 767) {
				...
				if (android_chrome && !$('#sidebar').hasClass('sidebar-fixed-responsive')) {
					...
					if ($(window).width() <= 767) {
						...
						$('#sidebar').css('margin-left', '-250px').css('margin-top', '0px');
					}
				} else {
					var wheelStepInt = 7;
