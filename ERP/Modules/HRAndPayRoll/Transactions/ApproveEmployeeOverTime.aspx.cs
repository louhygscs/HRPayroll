...
{
    public partial class ApproveEmployeeOverTime : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            SessionHelper.SelectMenuSession = "liApproveEmployeeOverTime_liHR_liHRTransactions";
                    ...
                    if (!string.IsNullOrEmpty(ddlMonth.SelectedValue))
                    {
                        ...
                        if (_Result.IsSuccess)
                        {
                            ...
                            for (DateTime date = _StartDate; date <= _EndDate; date = date.AddDays(1.0))
                            {
                                ...
                                if (_EmployeeAttendanceResult.OvertimeHours > 0)
                                {
        ...
        protected void chkApprove_CheckedChanged(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow rw in gvDeviceAttendance.Rows)
            {
                CheckBox chkBx = (CheckBox)rw.FindControl("chkApprove");
                if (chkBx != null && chkBx.Checked)
                {
                    Label _lblDate = (Label)gvDeviceAttendance.Rows[0].Cells[0].FindControl("lblAttendanceDate");
                    var _Date = _lblDate.Text;
                    var _OvertimeHours = gvDeviceAttendance.Rows[0].Cells[4].Text.ToString();
                    chkBx.Visible = false;
                    _EmployeeAttendances.EmployeeId = Guid.Parse(ddlEmployee.SelectedValue.ToString());
                    _EmployeeAttendances.OverTimeHours = Convert.ToDecimal(_OvertimeHours);
                    _EmployeeAttendances.WorkingHours = Convert.ToDecimal(gvDeviceAttendance.Rows[0].Cells[3].Text.ToString());
                    _EmployeeAttendances.FinancialYearId = SessionHelper.SessionDetail.FinancialYearId;
                    Result<Shift> _ResultShift = _IShiftService.GetShiftByEmployeeId(_EmployeeAttendances.EmployeeId);
                    _EmployeeAttendances.TimeIn = _ResultShift.Data.FromTime;
                    _EmployeeAttendances.TimeOut = _ResultShift.Data.ToTime;
                    _EmployeeAttendances.AttendanceDate = GlobalHelper.StringToDate(_Date);
                    Result<bool> _Result = _IEmployeeAttendanceService.SaveOverTime(_EmployeeAttendances);
                }
            }
        }
