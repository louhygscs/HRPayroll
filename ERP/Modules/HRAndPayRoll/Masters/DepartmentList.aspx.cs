        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_Result.IsSuccess)
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, typeof(Page), "DeletionSuccessMsg", " $(document).ready(function() {Common.ShowToastrMessage(Common.Variable.Success, Common.Variable.Success, '" + String.Format(GlobalMsg.DeletionSuccessMsg, "Department") + "');});", true);
                    gvDepartment_PreRender(gvDepartment, new EventArgs());
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this, typeof(Page), "DeletionFailMsg", " $(document).ready(function() {Common.ShowToastrMessage(Common.Variable.Error, Common.Variable.Error, '" + String.Format(_Result.Message, "Department") + "');});", true);
                }
            }
            catch (Exception _Exception)
            {
